...
Given "there are {int} completed todo items" do |count|
  ...
  count.times.each do |counter|
    ...
    find('label', text: "Test todo #{counter}").sibling('input', visible: false).set(true)
  end
end
...
Then("I should see mark all as complete button checked") do
  expect(find('input#toggle-all', visible: false)['checked']).to eql('true')
end
...
Then("I should see mark all as complete button unchecked") do
  expect(find('input#toggle-all', visible: false)['checked']).not_to be_truthy
end
